...
[[nodiscard]] std::optional<std::tuple<Matrix, int, int, bool, int>> read_SM() {
    ...
    std::cin >> eps;
    if (eps < 0) {
        std::string msg = "Approximation accuracy is less than zero";
        impossible_case(msg);
        return std::nullopt;
    }
    ...
    std::cout << std::endl;
    return std::make_optional(std::make_tuple(matrix, number_of_vars, number_of_equations, is_max_problem, eps));
}
